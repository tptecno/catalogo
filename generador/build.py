...
EXC = {"codigo", "código", "colores", "mayorista", "unit cost", "profit", "tato",
       "melman", "otros", "prov otro", "belgrano", "precio ars", "precio usd"}
...
def columna_colores(cab, di):
    """Cuál es la columna de colores de este bloque.

    Lo normal es que tenga el título "Colores" (Celulares, MacBook, iPad). En Watch
    la columna existe pero el encabezado está vacío, así que se acepta la que está
    pegada a la izquierda de la descripción si no es la del código."""
    for j, x in enumerate(cab):
        if x.lower() == "colores":
            return j
    j = di - 1
    return j if j > 0 and not cab[j] else -1
...
def parse_costos(rows):
    ...
    items, grupo, pi, di, ci = [], None, -1, -1, -1
    colores = []
    for raw in rows:
        ...
        if not grupo or pi < 0:
            continue
        # Los colores vienen solo en la primera fila del bloque y valen para todas.
        if ci > -1 and c[ci] and "USD" not in c[ci].upper():
            colores = [x.strip() for x in c[ci].split("\n") if x.strip()]
...
def escribir_sync(salida, filas):
    ...
    with open(ruta, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["categoria", "bloque", "codigo", "descripcion", "precio",
                    "hay_stock", "stock_texto", "specs"])
        w.writerows(filas)
...
def main():
    ...
    for key, label, pestana, parser, gid in CATS:
        if parser == "win":
            ...
            for p in prods:
                # Melman marca el stock con STK/ING/CONS; solo STK significa que hay.
                hay = "SI" if p["stock"].strip().upper() == "STK" else "NO"
                sync.append([key, p["bloque"], p["codigo"], p["desc"],
                             p["precio"], hay, p["stock"], p["specs"]])
        ...
        for i in items:
            sync.append([key, i["model"], i.get("codigo", ""), i["desc"],
                         i.get("price", ""), "SI" if i["avail"] else "NO",
                         "" if i["avail"] else i.get("estado", "Sin stock"), ""])
        # Los colores salen de la columna Colores de costos. Donde todavía no están
        # cargados se cruza por código contra el catálogo viejo, para no perderlos
        # mientras dure la migración. Cuando `heredados` llegue a 0 en todas las
        # categorías, `colores_publicados` y el gid se pueden borrar.
        propios = sum(1 for i in items if i.get("colors"))
        heredados = 0
        faltan = [i for i in items if not i.get("colors")]
        if faltan and gid:
            col = colores_publicados(gid)
            for i in faltan:
                c = col.get(i["codigo"])
                if c:
                    i["colors"] = c
                    heredados += 1
        pegados = propios + heredados
        for i in items:
            i.pop("codigo", None)
        datos[key] = items
        ...
        resumen.append(f"{label:18} {len(items):3} productos · "
                       ...
                       + (f" ({heredados} aún del catálogo viejo)" if heredados else ""))
    ...
    for fname, default in [("index.html", ""), ("iphone.html", "iphone"),
        ...
        # La portada se titula solo "Catálogo tptecno"; las de categoría la nombran.
        titulo = "Catálogo tptecno" if not default else f"Catálogo {lb} · tptecno"
        html = (tpl.replace("__TITULOPAG__", titulo)
                   .replace("__TITULO__", lb).replace("__CATS__", cats_js)
    ...
    demo = demo  # el título ya viene bien desde la plantilla
